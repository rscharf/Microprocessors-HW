...
volatile unsigned int sos[18] = {67,67,67,67,67,201,201,67,201,67,201,201,67,67,67,67,67,469};
unsigned int i = 0;
...
interrupt void WDT_interval_handler(){
  if (--blink_counter==0){          // decrement the counter and act only if it has reached 0
	  ...
	  blink_counter=sos[i];		//blink_interval; // reset the down counter
	  i++;
	  if (i == 18)
	  {
		  i = 0; //reset loop to beginning of sos message
	  }
  }
}
